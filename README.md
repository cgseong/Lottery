...
| 3 | 고급 추천 (고정번호 지정) |
| 4 | 저장된 조합 관리 및 품질 검토 |
| 5 | AI + 미출현 패턴 기반 고유 추천 |
| 6 | 종합 패턴 분석 추천 (10개 지표) |
| 7 | 전체 패턴 분석 기반 번호 추천 |
| 8 | 통합 AI 추천 (마르코프+군집+DL+필터) |
| 9 | 고급 분석 리포트 (핫/콜드/전이/군집) |
| 10 | 모든 회차정보 보기 |
...
### 2025-05-24
- **제외 번호 관리 기능 제거**
  - 메뉴 3번 "제외 번호 관리" 삭제 및 메뉴 번호 재배치 (11개 → 10개)
  - `analyzers/exclude_number_manager.py` 모듈 삭제
  - `exclude_numbers.json` 데이터 파일 삭제
  - 고급 추천 메뉴에서 제외번호 관련 로직 제거 (고정번호만 유지)
  - AI 패턴 추천에서 제외번호 참조 제거
